...
namespace SellWebsite.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public IActionResult Upsert(int? id)
        {
            ...
            {
                productVM.Product = _unitOfWork.Product.Get(p => p.ProductId == id, p => p.Categories!);
                if (productVM.Product.Categories != null)
                {
                    foreach (var item in productVM.Product.Categories)
                    {
                        productVM.CategoryList.First(p => p.Id == item.CategoryId).IsSelected = true;
                    }
                }
            }
        ...
        public IActionResult Upsert(ProductVM productVM, IFormFile? file)
        {
            ...
            else
            {
                productVM.CategoryList = _unitOfWork.Category.GetAll().Select(u => new CategoryListVM()
                {
                    Id = u.CategoryId,
                    Name = u.Name,
                    IsSelected = productVM.CategoryIdList.Contains(u.CategoryId),
                }).ToList();
            }
        ...
        public IActionResult GetAll()
        {
            ...
            List<Product> products = _unitOfWork.Product
                ...
                .Select(p =>
                        {
                            p.Categories ??= new List<Category>() { new() { Description = "" } };
                            return p;
                        }
            ).ToList();
